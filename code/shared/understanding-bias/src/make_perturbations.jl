...
# Find and load corpus
# C0 prefix means simplewiki, other prefix means nyt?
...
# Different perturbation sizes depending on corpus
...
"""
Loads all diff bias results matching the target pattern and returns the
mean and std_deviation of diff_bias (for a given bias test), for each
document.

Return value is a dataframe with columns [:doc_number, :ΔBIF_μ, :ΔBIF_σ]
"""
function load_df(target, bias_col=:ΔBIF_1)
    ...
    # read every csv matching target pattern in the diff_bias_dir,
    # adding the diff_bias scores to a new column in df
    for f in readdir(diff_bias_dir)
        if occursin(Regex(target), f)
            i += 1
            # read the diff_bias csv and sort it by doc_number
            tmp = sort(CSV.read(joinpath(diff_bias_dir, f), DataFrame),
                       :doc_num)
            # initialize document numbers based on first diff_bias file
            if (i == 1)
    ...
    # calculate a mean and standard deviation of the differential bias
    # for each document
